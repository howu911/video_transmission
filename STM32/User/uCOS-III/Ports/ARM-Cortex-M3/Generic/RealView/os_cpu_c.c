...
{
#if OS_CFG_APP_HOOKS_EN > 0u                               //如果使能（默认使能）了特殊用途钩子
    if (OS_AppTimeTickHookPtr != (OS_APP_HOOK_VOID)0) {    //如果指针 OS_AppTimeTickHookPtr 非空
        (*OS_AppTimeTickHookPtr)();                        //调用该指针存放的函数（地址）
    }
#endif
}
